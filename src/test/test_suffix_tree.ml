...
let test_exact_match_st test_ctxt =
  let pattern = "hey" in
  let text = "hey hey hey$" in
  let st = create text in
  let matches = exact_match_st st pattern in
  assert_equal [0; 4; 8] matches

let test_exact_match test_ctxt =
  let pattern = "hey" in
  let text = "hey hey hey$" in
  let matches = exact_match pattern text in
  assert_equal [0; 4; 8] matches
...
let suite =
  "suite" >:::
    [
      ...
      "test_exact_match_st" >:: test_exact_match_st;
